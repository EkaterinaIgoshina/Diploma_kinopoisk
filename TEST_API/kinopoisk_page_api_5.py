...
class CinemaPage:
    ...
    def get_movie_by_id(self, movie_id):
        with allure.step(f"Запрос фильма по ID: {movie_id}"):
            try:
                payload = {"movie_id": movie_id}  # Подготовка данных для POST-запроса
                response = requests.post(f'{self.base_url}v1.4/movie', json=payload, headers=self.my_headers)
                ...
                with allure.step(f"Проверка статус кода {response.status_code}"):
                    if response.status_code == 200:
                        ...
                        return None  # Возвращаем None для всех других статусов
